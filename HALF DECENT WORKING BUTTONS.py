...
Downloads CRX, patches it, writes request.js + backend settings, starts local CFC proxy.
Run: python main.py

Architecture matches cocodem EXACTLY:
  - Our code written to assets/request.js (replacing cocodem's or original)
  - Two lines injected into assets/index-BVS4T5_D.js (React jsx-runtime shim)
  - Service worker, HTML files, inline scripts: PATCHED FOR CSP
  - arc.html created (cocodem addition, not in official CRX)
  - Backend Settings button injected via MutationObserver on options.html
"""
...
def write_hijack_js():
    """Write our request.js to assets/request.js — replaces cocodem's version."""
    code = r'''// request.js — cocodem request.js structure, cfcBase = local proxy.
...
'''
    ...
    # Write the settings UI logic to a separate file to avoid CSP issues
    ui_code = r'''(async () => {
  const container = document.getElementById("__cfc_settings_container");
  if (!container) return;
  container.innerHTML = `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#1a1a2e;color:#e0e0e0;padding:40px;display:flex;justify-content:center;margin:0;min-height:100vh">
...
<div id="status" style="padding:12px;border-radius:8px;margin-bottom:20px;display:none;font-size:14px"></div>
<label style="display:block;margin-bottom:6px;font-weight:600;color:#ccc">Backend URL</label>
<input type="text" id="backendUrl" style="width:100%;padding:12px;border:1px solid #333;border-radius:8px;background:#16213e;color:#fff;font-size:14px;margin-bottom:20px;font-family:monospace;box-sizing:border-box">
<label style="display:block;margin-bottom:6px;font-weight:600;color:#ccc">Model Aliases (JSON, optional)</label>
<textarea id="modelAliases" style="width:100%;padding:12px;border:1px solid #333;border-radius:8px;background:#16213e;color:#fff;font-size:14px;margin-bottom:20px;font-family:monospace;box-sizing:border-box;min-height:100px;resize:vertical"></textarea>
<div style="display:flex;align-items:center;gap:10px;margin-bottom:20px">
  <input type="checkbox" id="blockAnalytics" checked style="width:auto;margin:0">
  <label style="margin:0;font-weight:600;color:#ccc">Block analytics / telemetry</label>
</div>
<button id="saveBtn" style="padding:12px 24px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;margin-right:10px;background:#e07a5f;color:#fff">Save Settings</button>
<button id="testBtn" style="padding:12px 24px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;margin-right:10px;background:#333;color:#fff">Test Connection</button>
<div style="margin-top:30px;padding:20px;border-radius:8px;background:#16213e;border:1px solid #333">
  <p><strong style="color:#e07a5f">How it works:</strong> All Claude API calls redirect to your backend. No login. No external domains. Everything stays local.</p>
  ...
  const s = { backendUrl: "http://127.0.0.1:1234/v1", modelAliases: {}, blockAnalytics: true, ...(hijackSettings || {}) };
  $("backendUrl").value = s.backendUrl;
  $("modelAliases").value = Object.keys(s.modelAliases).length ? JSON.stringify(s.modelAliases, null, 2) : "";
  $("blockAnalytics").checked = s.blockAnalytics;

  $("saveBtn").onclick = async () => {
    let modelAliases = {};
    try { if ($("modelAliases").value.trim()) modelAliases = JSON.parse($("modelAliases").value); }
    catch(e) { showStatus("Invalid JSON", true); return; }
    const settings = { backendUrl: $("backendUrl").value.trim(), modelAliases, blockAnalytics: $("blockAnalytics").checked };
    await chrome.storage.local.set({ hijackSettings: settings });
    showStatus("Saved!");
  };
  $("testBtn").onclick = async () => {
    try {
      const resp = await fetch($("backendUrl").value.replace(/\/v1\/?$/, "") + "/v1/models");
      if (resp.ok) showStatus("Connected!");
      else showStatus("Error: " + resp.status, true);
    } catch(e) { showStatus("Failed to connect", true); }
  };
})();'''
...
def write_arc_html():
    """Create arc.html — exists in cocodem zip but NOT in official CRX.
    Loaded programmatically via chrome.tabs.create({ url: '/arc.html' }) as
    Arc browser sidepanel fallback. Loads request.js directly as module."""
    html = '''<!doctype html>
<html lang="en">
...
def inject_index_module():
    """Inject setJsx into React jsx-runtime index module. Cocodem's exact method."""
    ...
    var_match = re.search(r',(\w)=c\(\),', content)
    if var_match:
    ...
    if boundary:
        ...
        if fn_match:
            ...
            print(f"[ERROR] Could not find injection point")
            sys.exit(1)
...
def write_options():
    # Remove inline scripts from sidepanel.html and options.html to fix CSP
    for html_file in [OUTPUT_DIR / "sidepanel.html", OUTPUT_DIR / "options.html"]:
        if html_file.exists():
            content = html_file.read_text(encoding="utf-8")
            # Remove <script>...</script> blocks that are not type="module"
            # This fixes the "Executing inline script violates CSP" error
            content = re.sub(r'<script>(.*?)</script>', '', content, flags=re.DOTALL)
            html_file.write_text(content, encoding="utf-8")
            print(f"[OK] Patched {html_file.name} for CSP (removed inline scripts)")
...
loadSettings();
'''
    # Real settings page is served by CFC proxy at /backend_settings
    # Static files above are kept for reference but not used — extension links go to the proxy
    redirect_stub = f'''<!DOCTYPE html><html><head><meta charset="utf-8">
<meta http-equiv="refresh" content="0;url=http://localhost:{CFC_PORT}/backend_settings">
<title>Backend Settings</title></head>
<body><script>window.location.href="http://localhost:{CFC_PORT}/backend_settings";</script></body></html>'''
    (OUTPUT_DIR / "backend_settings.html").write_text(redirect_stub, encoding="utf-8")
    print(f"\n[OK] backend_settings.html (redirect stub -> proxy :8520/backend_settings)")
...
def build_backend_settings_html():
    """Backend settings page served by the CFC proxy at /backend_settings.
    Uses chrome.runtime.sendMessage to read/write chrome.storage.local from this HTTP page."""
    ext_id = EXTENSION_ID
    default_url = DEFAULT_BACKEND_URL
    return f'''<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Backend Settings</title>
<style>
body{{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#1a1a2e;color:#e0e0e0;padding:40px;display:flex;justify-content:center;margin:0}}
.container{{max-width:600px;width:100%}}
h1{{font-size:28px;margin-bottom:8px;color:#fff}}
.subtitle{{color:#888;margin-bottom:30px}}
label{{display:block;margin-bottom:6px;font-weight:600;color:#ccc}}
input,textarea{{width:100%;padding:12px;border:1px solid #333;border-radius:8px;background:#16213e;color:#fff;font-size:14px;margin-bottom:20px;font-family:monospace;box-sizing:border-box}}
input:focus,textarea:focus{{outline:none;border-color:#e07a5f}}
textarea{{min-height:100px;resize:vertical}}
.checkbox-row{{display:flex;align-items:center;gap:10px;margin-bottom:20px}}
.checkbox-row input{{width:auto;margin:0}}
.btn{{padding:12px 24px;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;margin-right:10px}}
.btn-save{{background:#e07a5f;color:#fff}}.btn-save:hover{{background:#c96a50}}
.btn-test{{background:#333;color:#fff}}.btn-test:hover{{background:#444}}
.status{{padding:12px;border-radius:8px;margin-bottom:20px;display:none;font-size:14px}}
.status.success{{display:block;background:#1b4332;color:#95d5b2}}
.status.error{{display:block;background:#3d0000;color:#ff6b6b}}
.info-box{{margin-top:30px;padding:20px;border-radius:8px;background:#16213e;border:1px solid #333}}
.info-box p{{margin:8px 0}}
.info-box code{{background:#0d1117;padding:3px 8px;border-radius:4px;font-family:monospace;color:#7ee8fa}}
</style></head>
...
async function loadSettings() {{
  try {{
    ...
    const s = {{ backendUrl: DEFAULT_URL, modelAliases: {{}}, blockAnalytics: true, ...(r?.hijackSettings || {{}}) }};
    $("backendUrl").value = s.backendUrl;
    $("modelAliases").value = Object.keys(s.modelAliases).length ? JSON.stringify(s.modelAliases, null, 2) : "";
    $("blockAnalytics").checked = s.blockAnalytics;
  }} catch(e) {{
    $("backendUrl").value = DEFAULT_URL;
    showStatus("Could not read from extension — make sure extension is loaded and running.", true);
  }}
}}
...
async function saveSettings() {{
  let modelAliases = {{}};
  const raw = $("modelAliases").value.trim();
  if (raw) {{ try {{ modelAliases = JSON.parse(raw); }} catch {{ showStatus("Invalid JSON in model aliases", true); return; }} }}
  const settings = {{ backendUrl: $("backendUrl").value.trim() || DEFAULT_URL, modelAliases, blockAnalytics: $("blockAnalytics").checked }};
  try {{
...
def build_uinodes():
    settings_url = CFC_BASE + "backend_settings"
    return [
        {
            "selector": {
                "type": "div",
...
<a href="javascript:void(0)" onclick="try{{chrome.runtime.sendMessage('{EXTENSION_ID}', {{type: '_create_tab', url: 'http://localhost:' + String(8520) + '/backend_settings'}})}}catch(e){{}}"
...
def main():
    ...
    m = read_manifest()
    m = patch_manifest(m)
    write_hijack_js()          # writes our code to assets/request.js
    write_options()            # writes backend_settings.html/css/js + patches for CSP
    write_arc_html()           # creates arc.html (cocodem addition, not in official CRX)
    inject_index_module()      # injects setJsx into index-*.js (React jsx-runtime)
